...
import { Camera, User, ArrowRight, Phone, Loader2 } from "lucide-react";
...
const SetupTalentProfile = () => {
  ...
  const [cover, setCover] = useState<{ file: File | null; preview: string }>({
    file: null,
    preview: "https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?q=80&w=1000",
  });
  ...
  const handleFinish = async () => {
    ...
    if (user?.employeeId) formData.append("employeeId", user.employeeId.toString());

    try {
      setLoading(true);
      const response = await fetch("https://userprofile-service.grayforest-11aba44e.southeastasia.azurecontainerapps.io/api/Employee", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        // Redirect hoặc đóng modal ở đây
      } else {
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 overflow-y-auto no-scrollbar">
      ...
      <div className="relative z-10 bg-white w-full max-w-[420px] rounded-[3rem] shadow-[0_32px_64px_-15px_rgba(0,0,0,0.3)] overflow-hidden border border-white/10 animate-in zoom-in-95 duration-500 flex flex-col">
        ...
        <div className="p-8 pb-4 text-center">
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Chào mừng người mới</h1>
          <p className="text-[12px] text-slate-500 mt-2 leading-relaxed px-4 font-medium italic">
        ...
        <div className="relative h-36 shrink-0">
          ...
          <input type="file" hidden ref={coverInputRef} accept="image/*" onChange={(e) => handleImageChange(e, 'cover')} />
          ...
          <div className="absolute -bottom-8 left-1/2 -translate-x-1/2">
            ...
            >
              {avatar.preview ? (
                <img src={avatar.preview} className="w-full h-full object-cover" />
              ) : (
            ...
            <input type="file" hidden ref={avatarInputRef} accept="image/*" onChange={(e) => handleImageChange(e, 'avatar')} />
          </div>
        </div>
        ...
        <div className="p-8 pt-12 space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] ml-1">Tên hiển thị *</label>
          ...
          <button 
            ...
            className="w-full py-4 mt-2 bg-[#0288D1] hover:bg-[#039BE5] text-white rounded-2xl font-black text-sm shadow-lg shadow-blue-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed group"
          >
            {loading ? (
              <Loader2 className="animate-spin" size={18} />
            ) : (
              <>
                HOÀN THÀNH <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
              </>
            )}
          </button>
        </div>
