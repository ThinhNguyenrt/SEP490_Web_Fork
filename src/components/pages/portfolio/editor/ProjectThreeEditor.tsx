...
import { type ProjectThreeDraft } from "./projectThreeDraft";

type ProjectThreeEditorProps = {
  initialData: ProjectThreeDraft;
  onSave: (nextDraft: ProjectThreeDraft) => void;
  onCancel: () => void;
};

export default function ProjectThreeEditor({
  initialData,
  onSave,
  onCancel,
}: ProjectThreeEditorProps) {
  const [draft, setDraft] = useState<ProjectThreeDraft>(initialData);
  ...
  const updateDraftField = (field: keyof ProjectThreeDraft, value: string) => {
    setDraft((prevDraft) => ({
      ...prevDraft,
      [field]: value,
    }));
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[#d7dfeb] bg-[#EFF6FF]">
      <div className="flex items-start justify-between border-b border-[#d7dfeb] px-4 py-3">
        <div>
          <h3 className="text-[30px] font-bold leading-tight text-slate-800">Dự án và đề tài</h3>
          <p className="mt-1 text-sm text-slate-500">
            Hãy điền thông tin dự án và đề tài để hiển thị trong hồ sơ của bạn
          </p>
        </div>
        <button
          type="button"
