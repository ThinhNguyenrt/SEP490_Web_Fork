...
const OtherFour: React.FC<OtherFourProps> = ({ data }) => {
    ...
    <div className="otherinfo-block bg-white px-6 py-8 border-b border-gray-200 last:border-b-0">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center">
          <img src={HumanBrainIcon} alt="Tổng quan nghiên cứu" className="w-6 h-6" />
        </div>
        <h3 className="text-xl font-bold text-gray-900">Giới thiệu chuyên môn</h3>
      </div>

      {detail ? (
        <p className="text-base leading-relaxed text-slate-900">{detail}</p>
      ) : (
        <p className="text-sm text-slate-500">Chưa cập nhật tổng quan nghiên cứu</p>
      )}
    </div>
  );
};
